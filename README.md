...
2. Once installed, you need to authenticate with your Snyk account by running `snyk auth`
3. Run `snyk wizard` in the directory of your project following the prompts which will also generate a `.snyk` policy file.
4. Ensure the `.snyk` file you generated was added to your source control (`git add .snyk`).
5. If you selected to, Snyk will include `snyk test` as part of your `npm test` command, so if there are new vulnerabilities in the future, your CI will fail protecting you from introducing vulnerabilities to production.
